...
function head($title){ 
	...
	print "<script type='text/javascript' language='javascript' src='_tr/func_js.js' charset='utf-8'></script> \r\n"; 
...
function bt_voltar($lb, $link){
	print "<a href='".$link."' class='btg' id='bt_voltar'><span><b class='arrow'>&larr;</b> ".$lb." </span></a>";
} 
...
function bt_gravar($lb){ 
	print "<input id='bt_gravar' type='submit' class='btg' value='".$lb."'/>\r\n";
} 
...
function select($id, $cls, $name, $onchange, $multiple, $opt, $sel, $opt0){ 
	...
	print "<img id='submenu_select_ico' src='_layout/bg_select_cms2.png' />\r\n";
...
function menu( $menu_arr, $lg_arr, $menu_ativo ){	
	...
	//idioma 
	if(count($lg_arr)>0){  
		//select($id, $name, $onchange, $multiple, $opt, $sel, $opt0
	...
	print "<input type='hidden' id='menu_ativo' value='".$menu_ativo."'/>\r\n";
	print "<script type='text/javascript'>resize();</script>\r\n";
	print "</div>\r\n";
}  
	
function menu_li($lb, $url, $sel){ 
	if($sel){
		print "<li class='menu_atual'>".$lb."</li>";
	}else{
		print "<li><a target='_self' href='".$url."'>".$lb."</a></li>";
	}
} 
...
function submenu( $submenu_arr, $submenu_ativo ){ 
	...
	print "<div id='submenu_list'>";
	
	$border = 'bl';
	
	for($i=0; $i<count($submenu_arr); $i++){
		if($i>0) $border = '';
		($submenu_ativo==$i+1) ? (submenu_bt("",$border,$submenu_arr[$i][0],"",true) ) : (submenu_bt("",$border,$submenu_arr[$i][0],$submenu_arr[$i][1],false));
	}
	
	print "</div>\r\n";   
	select( 'submenu_select', '', '', '', false, $submenu_arr, array($submenu_ativo), false  );
	//select($id, $cls, $name, $onchange, $multiple, $opt, $sel, $opt0)
		  
	print "</div>\r\n"; 
}  

function submenu_bt( $id, $cls, $lb, $url, $sel ){ 
	if($sel){
		print "<a id='".$id."' class='submenu_atual ".$cls."'>".$lb."</a>";
	}else{  
		print "<a id='".$id."' class='".$cls."' href='".$url."'>".$lb."</a>";
	}
} 
...
function band( $lg_ico ){
	...
	if($lg_ico) return "<img width='11' src='_layout/".lg("bandeira").".jpg'/>";
}
