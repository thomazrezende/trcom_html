...
function replace_arq( $conexao, $label, $path_file, $tabela, $campo, $condicao ){
...
function up_img_fixo($label,$id,$wmax,$hmax,$arqfim,$tipo){ 
	...
	if (!empty($nome_arquivo)){ 
			...
			switch($tipo){
				case "gif": $ok = imagegif($tmp,$arqfim.".gif"); break;
				case "jpeg": $ok = imagejpeg($tmp,$arqfim.".jpg",$jpg_q); break;
				case "png": $ok = imagepng($tmp,$arqfim.".png"); break;
			}
	...
	if (!empty($nome_arquivo)){		
		...
		/////// IMG
			...
			switch($tipo){
				...
				case "jpeg": $ok_img = imagejpeg($tmp_img,$arqfim.".jpg",$jpg_q); break;
...
function remove_arq( $conexao, $path, $arquivo, $tabela, $id, $label){
...
function remove_item( $conexao, $path, $item, $tabela, $id){
...
function remove_itens( $conexao, $path, $itens, $tabela, $id){
...
function remove_pasta( $conexao, $path, $tabela, $id, $tabelas_vinc, $lbs_vinc){ 
	/* //
		...
		if($consulta){				
			...
			if(isset($tabelas_vinc) && !empty($tabelas_vinc)){
				for($i=0; $i<count($tabelas_vinc); $i++){
					...
					$consulta_vinc = $conexao->query($sql_vinc);
				}
			}
			rmdir($path);
			return true;								
